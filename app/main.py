...
from app.launch_agent import install_launch_agent, remove_launch_agent, is_launch_agent_installed
...
class ZekatMenubarApp(rumps.App):
    ...
    def __init__(self, port: int = 8000):
        ...
        self.launch_at_login_item.state = is_launch_agent_installed()
    ...
    def toggle_launch_at_login(self, sender):
        """Toggle the LaunchAgent for opening at login."""
        if sender.state:
            if remove_launch_agent():
                sender.state = False
                logger.info("Launch at Login disabled")
            else:
                rumps.alert("Failed to disable Launch at Login. Check logs for details.")
        else:
            if install_launch_agent():
                sender.state = True
                logger.info("Launch at Login enabled")
            else:
                rumps.alert("Failed to enable Launch at Login. Check logs for details.")
